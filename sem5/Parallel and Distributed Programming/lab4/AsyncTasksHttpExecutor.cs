...
namespace AsyncTasks
{
    public class AsyncTasksHttpExecutor
    {
        ...
        private static async Task ReceiveData(RequestWrapper request) {
            request.Socket.BeginReceive(request.Buffer, 0, RequestWrapper.BufferSize, 0, 
                ReceiveCallback, request);
        ...
        private static void ReceiveCallback(IAsyncResult ar)
        {
            ...
            try {
                ...
                if (!HttpParser.ResponseHeaderObtained(state.ResponseContent.ToString())) {
                    clientSocket.BeginReceive(state.Buffer, 0, RequestWrapper.BufferSize, 0, 
